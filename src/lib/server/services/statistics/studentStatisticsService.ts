...
import { ItemInspectionStatus } from '$lib/server/models/BorrowRequest';
...
function computeReturnPerformance(requests: BorrowRequest[]): ReturnPerformance {
	...
	for (const req of returned) {
		// returnedAt is set by the /return endpoint directly.
		// When the /inspect-items path transitions status to RETURNED, it writes
		// `$set: { returnedAt: condition ? now : undefined }` — MongoDB silently
		// ignores undefined values in $set, so returnedAt may be absent on those
		// records. Fall back to updatedAt (always stamped on every transition)
		// so we never drop the request into the unknown bucket.
		const returnTimestamp = req.returnedAt ?? req.missingAt ?? req.updatedAt;
...
function computeItemHealth(requests: BorrowRequest[], allObligations: ReplacementObligation[]): ItemHealthStats {
	...
	// Obligation lookup: "borrowRequestId_itemId" → obligation, used to
	// upgrade damaged/missing items that have since been replaced.
	...
	for (const req of requests) {
		...
		const hasAnyInspection = req.items.some((item) => !!item.inspection);

		if (!hasAnyInspection) {
			// Requests completed via the direct /return endpoint bypass the
			// /inspect-items step entirely — the custodian accepted all items
			// without recording per-item inspection data. Treat every item as
			// good-condition so they are represented in the health metric.
			if (req.status === 'returned') {
				goodCondition += req.items.length;
				totalInspected += req.items.length;
			}
			continue;
		}
		...
		for (const item of req.items) {
			...
			if (status === 'damaged' || status === 'missing') {
				...
				if (obl && obl.status === 'replaced') {
					// Replaced items are back in inventory — count as good.
					status = ItemInspectionStatus.GOOD;
				}
			}
