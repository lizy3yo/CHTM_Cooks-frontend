<script lang="ts">
	...
	// readOnly hides every create/process action (admin is view-only).
	interface Props {
		role: 'instructor' | 'custodian' | 'admin' | 'superadmin';
		readOnly?: boolean;
	}

	let { role, readOnly = false }: Props = $props();

	const roleLabel = $derived(
		{ instructor: 'Instructor', custodian: 'Custodian', admin: 'Admin', superadmin: 'Superadmin' }[role]
	);
	...
	// ─── PAGINATION (10 per page on every tab) ───────────────────────────────
	const PAGE_SIZE = 10;
	let walkInPage = $state(1);
	let confidentialPage = $state(1);
	let donationPage = $state(1);

	const pageCount = (total: number) => Math.max(1, Math.ceil(total / PAGE_SIZE));
	const pageSlice = <T,>(list: T[], page: number) =>
		list.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);

	const walkInPages = $derived(pageCount(displayWalkIns.length));
	const confidentialPages = $derived(pageCount(displayConfidentialRequests.length));
	const donationPages = $derived(pageCount(filteredDonations.length));

	const pagedWalkIns = $derived(pageSlice(displayWalkIns, walkInPage));
	const pagedConfidential = $derived(pageSlice(displayConfidentialRequests, confidentialPage));
	const pagedDonations = $derived(pageSlice(filteredDonations, donationPage));

	// Back to page 1 whenever a tab's search or filters change.
	$effect(() => {
		walkInSearchQuery;
		walkInStatusFilter;
		walkInPersonFilter;
	...
	$effect(() => {
		confidentialSearchQuery;
		confidentialStatusFilter;
		confidentialPage = 1;
	});
	$effect(() => {
		donationSearchQuery;
	...
	// Keep each page in range when records are returned, resolved or refreshed.
	$effect(() => {
		if (walkInPage > walkInPages) walkInPage = walkInPages;
		if (confidentialPage > confidentialPages) confidentialPage = confidentialPages;
		if (donationPage > donationPages) donationPage = donationPages;
	});
	...
	function isRegisteredBorrower(tx: WalkInTransaction): boolean {
		return /^\d+$/.test(tx.studentId) && Boolean(tx.email);
	}

	// Second line under the borrower name: the school email for students,
	// the guest ID and email (whichever exist) for guests. Never the internal account id.
	function borrowerLine(tx: WalkInTransaction): string {
		if (isRegisteredBorrower(tx)) return tx.email;
		const guestId = tx.studentId && tx.studentId !== 'GUEST' ? tx.studentId : '';
		return ['Guest', guestId, tx.email].filter(Boolean).join(' · ');
	}
...
{#snippet pageBar(page: number, pages: number, total: number, onChange: (p: number) => void)}
	{#if pages > 1}
		<Pagination
			currentPage={page}
			totalPages={pages}
...
{#snippet itemList(items: { name: string; quantity: number }[])}
	<ul class="space-y-0.5" title={itemsTooltip(items)}>
		{#each items.slice(0, MAX_ITEMS_SHOWN) as item}
			<li class="flex gap-1.5 text-xs text-gray-700">
				<span class="shrink-0 font-bold text-pink-600 tabular-nums">{item.quantity}×</span>
				<span class="truncate">{item.name}</span>
			</li>
		{/each}
		{#if items.length > MAX_ITEMS_SHOWN}
			<li class="text-xs font-medium text-gray-400">
...
<svelte:head><title>Alternative Transactions - {roleLabel}</title></svelte:head>

<div class="space-y-6">
	<!-- ─── HEADER ──────────────────────────────────────────────────────────── -->
	<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
		<div>
			<h1 data-tour="{role}-transactions-header" class="text-2xl font-bold text-gray-900 sm:text-3xl">Alternative Transactions</h1>
			<p class="mt-1 text-sm text-gray-500">
				{readOnly
					? 'Read-only overview of desk walk-ins, confidential admin requests and item donations.'
					: 'Dedicated workflows for desk walk-ins and confidential admin operations.'}
			</p>
		</div>

		<!-- Action buttons (hidden for view-only roles) -->
		<div class="flex gap-2">
			{#if readOnly}
				<span
					class="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-xs font-semibold text-gray-600"
				>
	...
	<!-- ─── STATS CARDS ─────────────────────────────────────────────────────── -->
	<div class="grid grid-cols-2 gap-3 sm:grid-cols-3" data-tour="{role}-transactions-stats">
	...
	<!-- ─── TABS ────────────────────────────────────────────────────────────── -->
	<div class="border-b border-gray-200" data-tour="{role}-transactions-tabs">
	...
	{#if activeTab === 'walk-in'}
		<div class="space-y-4">
					...
					<table class="hidden w-full table-fixed text-left @4xl:table">
						...
						>
							<tr>
								...
								{#if !readOnly}<th class="px-4 py-3 text-right">Action</th>{/if}
							</tr>
						</thead>
						<tbody class="divide-y divide-gray-100 text-sm text-gray-700">
							{#each pagedWalkIns as tx (tx.id)}
								<tr class="align-top transition-colors hover:bg-gray-50/60">
									...
									<td class="px-4 py-3.5">{@render itemList(tx.items)}</td>
									<td class="px-4 py-3.5">{@render walkInStatus(tx)}</td>
									{#if !readOnly}
										<td class="px-4 py-3.5 text-right">{@render walkInAction(tx)}</td>
									{/if}
								</tr>
							{/each}
						</tbody>
						...
						{#each pagedWalkIns as tx (tx.id)}
							<li class="space-y-3 p-4">
								...
								<div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-500">
									<span class="font-mono font-bold text-gray-900">{tx.id}</span>
									<span>{fmtDate(tx.borrowDate)} · {fmtTime(tx.borrowDate)}</span>
									{@render walkInClass(tx)}
								</div>
								{@render itemList(tx.items)}
			...
			{@render pageBar(walkInPage, walkInPages, displayWalkIns.length, (p) => (walkInPage = p))}
		</div>
	{/if}
						...
						<tbody class="divide-y divide-gray-100 text-sm text-gray-700">
							{#each pagedConfidential as req (req.id)}
								<tr class="align-top transition-colors hover:bg-gray-50/60">
									...
									<td class="px-4 py-3.5">{@render confPriority(req)}</td>
									<td class="px-4 py-3.5">{@render confStatus(req)}</td>
									{#if !readOnly}
										<td class="px-4 py-3.5 text-right">{@render confAction(req)}</td>
									{/if}
								</tr>
							{/each}
						</tbody>
	...
	{:else if activeTab === 'donations'}
		...
		<div class="space-y-4">
			...
			<div class="@container overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xs">
				...
				{:else if filteredDonations.length === 0}
					<div class="py-16 text-center">
						...
						<p class="mt-1 text-sm text-gray-500">
							{readOnly
								? 'Donations logged by custodians and instructors will appear here.'
								: 'Log a new item donation using the button above.'}
						</p>
					</div>
				{:else}
						...
						<tbody class="divide-y divide-gray-100">
							{#each pagedDonations as d (d.id)}
								<tr class="align-top transition-colors hover:bg-gray-50/60">
									...
									<td class="px-4 py-3.5">{@render donItem(d)}</td>
									<td class="px-4 py-3.5 text-right tabular-nums">
										<p class="font-bold text-gray-900">+{d.quantity}</p>
										<p class="text-xs text-gray-400">{d.unit || (d.quantity === 1 ? 'unit' : 'units')}</p>
									</td>
									<td class="px-4 py-3.5">{@render donAction(d)}</td>
									<td class="px-4 py-3.5">{@render donPurpose(d)}</td>
								</tr>
							{/each}
						</tbody>
					</table>
							...
							<li class="space-y-2.5 p-4">
								<div class="flex items-start justify-between gap-3">
									...
									<p class="shrink-0 text-right tabular-nums">
										...
										<span class="text-xs text-gray-400">{d.unit || (d.quantity === 1 ? 'unit' : 'units')}</span>
									</p>
								</div>
			...
			{@render pageBar(donationPage, donationPages, filteredDonations.length, (p) => (donationPage = p))}
		</div>
	{/if}
</div>
