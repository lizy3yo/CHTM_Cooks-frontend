<script lang="ts">
	...
	import { Moon, Sun, Bell, ChevronDown, LogOut, CalendarDays, User, Settings, History, HelpCircle } from 'lucide-svelte';
	import SignOutModal from '$lib/components/ui/SignOutModal.svelte';
	import logo from '$lib/assets/CHTM_LOGO.png';

	// Only render on auditor routes
	const isAuditorRoute = $derived($page.url.pathname.startsWith('/auditor'));
	...
	async function logout() {
		...
		toastStore.success('Logged out successfully.', 'Goodbye');
		goto('/auth/login');
	}
...
>
	<!-- Left: Branding on Mobile / Clock on Desktop -->
	<div class="flex items-center gap-2">
		...
		<div class="flex items-center gap-2 lg:hidden">
			...
			<div class="leading-tight">
				<p class="text-xs font-bold text-gray-900">Auditor</p>
			...
			{#if notifOpen}
				...
				>
					<div class="flex items-center justify-between border-b border-gray-100 px-4 py-3">
						...
						<a href="/auditor/notifications" onclick={() => notifOpen = false} class="text-xs font-medium text-pink-600 hover:text-pink-700">View all</a>
					</div>
						...
						<div class="max-h-80 overflow-y-auto py-2">
							{#each recentNotifications as notification (notification.id)}
								...
								<button
									...
									class="flex w-full items-start gap-3 border-l-2 px-4 py-3 text-left transition-colors hover:bg-gray-55 hover:bg-slate-50 transition-colors {notification.isRead ? 'border-transparent' : 'border-pink-500 bg-pink-50/40'}"
								>
			...
			<div class="relative" data-topnav-dropdown>
				...
				{#if profileOpen}
					...
					>
						...
						<a href="/auditor/account/profile" onclick={() => profileOpen = false} role="menuitem" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 transition-colors hover:bg-pink-50 hover:text-pink-600">
							<User size={15} class="text-gray-400" /> Profile
						</a>
						<a href="/auditor/account/settings" onclick={() => profileOpen = false} role="menuitem" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 transition-colors hover:bg-pink-50 hover:text-pink-600">
							<Settings size={15} class="text-gray-400" /> Settings
						</a>
						<a href="/auditor/history" onclick={() => profileOpen = false} role="menuitem" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 transition-colors hover:bg-pink-50 hover:text-pink-600">
