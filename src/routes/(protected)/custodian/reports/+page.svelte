<script lang="ts">
	...
	// Walk-in (Alternative) transactions now come from the backend analytics payload,
	// so every staff role sees the same records (scoped to the selected period).
	...
	const filteredLossDamageTracking = $derived.by(() => {
		...
		// Enrich rows with a computed `daysToResolve` value where possible.
	...
	const filteredInventoryVarianceRows = $derived.by(() => {
		...
		// Start from the canonical variance rows (already excludes zero-variance)
		let rows = inventoryVarianceRows.slice();
		// Apply Over/Under filter
		if (inventoryVarianceFilter === 'over') {
			rows = rows.filter((r) => r.variance > 0);
		} else if (inventoryVarianceFilter === 'under') {
			rows = rows.filter((r) => r.variance < 0);
		}
	...
	const filteredRelatedRequestRows = $derived.by(() => {
		...
		// Only include variance items that have a related driver with a real request context
		const candidates = inventoryVarianceRows.filter((item) => {
			const driver = driverById.get(item._id);
			if (!driver) return false;
			// Consider a driver "related" if it has a latestRequestId or at least one recorded request
			if (driver.latestRequestId && driver.latestRequestId.trim() !== '') return true;
			if ((driver.requestCount ?? 0) > 0) return true;
			return false;
		});
	...
	onMount(() => {
		...
		// If a `tab` query param was provided, use it to open the requested tab (industry standard deep-link)
		try {
			const tabParam = get(page).url.searchParams.get('tab');
			if (tabParam && ['overview', 'borrowing', 'loss-damage', 'inventory', 'students'].includes(tabParam)) {
				activeTab = tabParam as any;
				// If we're deep-linking directly to the Inventory tab, force a fresh analytics fetch
				// to ensure the Inventory Analytics / Variance Items / Related Requests show current system data.
				if (tabParam === 'inventory') {
					void loadReport(true);
					// skip the default cache-only load below
					return;
				}
			}
		} catch (e) {
...
<div class="space-y-6">
	<!-- Header -->
	<div class="flex items-start justify-between gap-4" data-tour="custodian-reports-header">
		<div>
	...
	<div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm space-y-4" data-tour="custodian-reports-filters">
	...
	<div class="rounded-xl border border-gray-200 bg-white shadow-sm" data-tour="custodian-reports-tabs">
											...
											{:else}
												<div class="space-y-2 overflow-y-auto p-3">
													{#each paginatedBorrowedItems as item}
														<button type="button" onclick={() => (borrowDetail = { kind: 'entry', subject: item })} class="w-full cursor-pointer rounded-lg border border-gray-200 bg-gray-50 p-3 text-left transition hover:border-pink-300 hover:bg-pink-50/40">
															<div class="flex items-center justify-between">
										...
										<div class="mt-3">
											...
											<div class="mt-3 flex items-center justify-between gap-3">
												<div class="flex items-center gap-2 text-xs text-gray-600">
													...
													<!-- Segmented control: All / Over / Under -->
													<div role="tablist" aria-label="Variance filter" class="inline-flex rounded-lg bg-gray-100 p-1">
														<button
															type="button"
															onclick={() => (inventoryVarianceFilter = 'all')}
															aria-pressed={inventoryVarianceFilter === 'all'}
															class="rounded-lg px-3 py-1 text-xs font-medium transition {inventoryVarianceFilter === 'all' ? 'bg-white text-pink-600 shadow-sm' : 'text-gray-600 hover:bg-gray-200'}"
														>
															All
														</button>
														<button
															type="button"
															onclick={() => (inventoryVarianceFilter = 'over')}
															aria-pressed={inventoryVarianceFilter === 'over'}
															class="rounded-lg px-3 py-1 text-xs font-medium transition {inventoryVarianceFilter === 'over' ? 'bg-white text-emerald-700 shadow-sm' : 'text-gray-600 hover:bg-gray-200'}"
														>
															Over
														</button>
														<button
															type="button"
															onclick={() => (inventoryVarianceFilter = 'under')}
															aria-pressed={inventoryVarianceFilter === 'under'}
															class="rounded-lg px-3 py-1 text-xs font-medium transition {inventoryVarianceFilter === 'under' ? 'bg-white text-rose-700 shadow-sm' : 'text-gray-600 hover:bg-gray-200'}"
														>
															Under
														</button>
													</div>
																...
																<div class="min-w-0 flex-1">
																	...
																	<p class="mt-2 text-xs text-gray-500">
																		{driver?.latestRequestDate
																			...
																			: 'No request date'}
																		{#if driver?.latestRequestStatus && driver.latestRequestStatus !== 'approved_instructor'}
																			&middot; {driver.latestRequestStatus}
																		{/if}
																	</p>
																</div>
																<div class="text-right">
