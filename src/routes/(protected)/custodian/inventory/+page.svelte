<script lang="ts">
	...
	function truncateName(name: string | undefined | null, maxLength = 19): string {
		if (!name) return '';
	...
	// Reset to page 1 when filter criteria change (not when underlying data changes)
	$effect(() => {
		query;
		selectedCategory;
		requiredFilter;
...
<div class="w-full min-w-0 space-y-4">
	...
	<div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
		<div class="min-w-0">
			<h1 data-tour="custodian-inventory-header" class="text-2xl font-bold text-gray-900 sm:text-3xl">Inventory Management</h1>
	...
	{:else}
		...
		{:else}
			<div class="grid grid-cols-2 gap-3 lg:grid-cols-4" data-tour="custodian-inventory-stats">
				<!-- Card 1: Total Lab Stock -->
				<button type="button" onclick={() => { stockModalMode = 'total'; showStockModal = true; }} class="rounded-lg border border-gray-200 bg-white p-3 shadow-sm transition-all duration-200 hover:border-pink-300 hover:shadow-md active:scale-98 cursor-pointer text-left focus:outline-none focus:ring-2 focus:ring-pink-500/20 sm:p-5">
				...
				<!-- Card 3: Out / Released -->
				<button type="button" onclick={() => (showReleasedModal = true)} class="rounded-lg border border-gray-200 bg-white p-3 shadow-sm transition-all duration-200 hover:border-pink-300 hover:shadow-md active:scale-98 cursor-pointer text-left focus:outline-none focus:ring-2 focus:ring-pink-500/20 sm:p-5">
					<div class="flex items-center justify-between gap-2">
						<div class="min-w-0">
				...
				<!-- Card 4: Flow Balance -->
				<button type="button" onclick={() => { stockModalMode = 'flow'; showStockModal = true; }} class="rounded-lg border border-gray-200 bg-white p-3 shadow-sm transition-all duration-200 hover:border-pink-300 hover:shadow-md active:scale-98 cursor-pointer text-left focus:outline-none focus:ring-2 focus:ring-pink-500/20 sm:p-5">
					<div class="flex flex-col justify-between h-full">
						<div class="flex items-center justify-between gap-2 mb-2">
							<p class="truncate text-xs font-medium text-gray-600 sm:text-sm">Stock Flow Balance</p>
							<span class="inline-flex items-center rounded-md bg-pink-100 px-1.5 py-0.5 text-[10px] font-semibold text-pink-700 ring-1 ring-pink-600/10">
				...
				{:else}
					...
					<div class="mb-4 flex flex-col gap-3">
						...
						<div class="flex flex-col gap-3">
							...
							<div class="relative w-full" data-tour="custodian-inventory-search">
								<input
									type="text"
										...
										<tbody class="divide-y divide-gray-100 bg-white">
											{#each processedImportPreviewData as item}
												...
												>
													...
													<td class="px-4 py-3 text-sm text-gray-900">{truncateName(item.name)}</td>
