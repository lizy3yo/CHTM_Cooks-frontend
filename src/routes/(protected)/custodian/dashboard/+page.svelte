...
const damageRateDisplay = $derived.by(() => {
	...
	function getRequestDeepLink(r: BorrowRequestRecord): string {
		...
		return `/custodian/requests?${params.toString()}`;
	}
...
<div class="space-y-6">
	...
	<!-- ── Header ─────────────────────────────────────────────────────────── -->
	<div class="flex items-start justify-between gap-3" data-tour="custodian-dash-header">
		<div class="min-w-0">
	...
	{:else}
		<!-- ── KPI strip ───────────────────────────────────────────────────── -->
		<div class="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-4" data-tour="custodian-dash-kpis">
	...
	{/if}
		...
		<div class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-gray-100" data-tour="custodian-dash-actions">
		...
		{:else}
			...
			<div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
				<div class="mb-4 flex items-center justify-between">
					...
					<a href="/custodian/reports?tab=inventory" class="flex items-center gap-1 text-xs font-medium text-pink-600 hover:text-pink-700">
			...
			<div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
				<div class="mb-4 flex items-center justify-between">
					...
					<a href="/custodian/reports?tab=students" class="flex items-center gap-1 text-xs font-medium text-pink-600 hover:text-pink-700">
