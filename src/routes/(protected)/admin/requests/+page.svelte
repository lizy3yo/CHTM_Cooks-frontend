			...
			<div class="grid grid-cols-2 gap-3 lg:grid-cols-5" data-tour="admin-requests-stats">
				...
				>
					<div class="flex items-center justify-between gap-2">
						...
						>
							<svg
								class="h-5 w-5 text-violet-600 sm:h-6 sm:w-6"
								fill="none"
								stroke="currentColor"
								viewBox="0 0 24 24"
							>
...
{#if showInspectionModal && selectedRequest}
	<ItemInspectionModal
		...
		leaderName={selectedRequest.instructorData?.fullName ?? selectedRequest.approvedBy}
		sessionDate={selectedRequest.borrowDate}
