<script lang="ts">
	...
	let selectedRequired = $state('all');
	...
	const requiredOptions = [
		...
		{ value: 'required', label: 'Required Items Only' }
	];
	...
	async function fetchCatalog(
		...
		try {
			...
			const baseFilters: CatalogFilters = {
				...
				required: (selectedRequired as CatalogFilters['required']) || 'all',
	...
	<div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
		...
		<div class="flex items-center gap-2">
			{#if hasNoEnrollment || availableClassCodes.length === 0}
				<button
					disabled
					data-tour="student-catalog-enroll"
	...
	<div class="rounded-lg bg-white p-3 shadow sm:p-4">
		...
		<div class="grid grid-cols-2 gap-2 sm:grid-cols-3">
			...
			<select
				...
				onchange={(e) => {
					selectedRequired = (e.target as HTMLSelectElement).value;
					handleFilterChange();
				}}
				class="block w-full rounded-lg border border-gray-300 px-2 py-2 text-sm focus:border-pink-500 focus:ring-pink-500"
				aria-label="Filter by required items"
				disabled={isLoading}
			>
				{#each requiredOptions as option}
		...
		<div class="rounded-xl border border-pink-200 bg-pink-50 p-4 shadow-sm animate-fadeIn">
			<div class="flex gap-3">
				...
				<div class="flex-1 min-w-0">
					...
					<p class="mt-1 text-xs text-pink-700 leading-relaxed">
						You already have a pending borrow request awaiting action. You cannot request equipment or modify item lists until your current request is processed.
					</p>
				</div>
			</div>
		</div>
