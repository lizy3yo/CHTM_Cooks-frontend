<script lang="ts">
	...
	// Keep the preview page valid when the item list changes, but do not force page 1.
	$effect(() => {
		const totalPages = previewItemsTotalPages;
		if (previewItemsPage > totalPages) {
			previewItemsPage = totalPages;
		}
	});
	...
	// Keep the selected-items page valid when the cart changes, but preserve the current page.
	$effect(() => {
		const totalPages = selectedItemsTotalPages;
		if (selectedItemsPage > totalPages) {
			selectedItemsPage = totalPages;
		}
	});
	...
	// Trust score gate: industry-standard eligibility check
	// Students with a Trust Score in the "Poor" (40-59) or "Critical" (<40)
	// ranges are temporarily blocked from submitting new requests.
	let trustScoreValue = $state<number | null>(null);
	...
	// Notify and block when trust score indicates poor/critical standing
	$effect(() => {
		if (!hasLowTrust) {
			hasShownTrustDialog = false;
			return;
		}

		if (hasShownTrustDialog) return;

		hasShownTrustDialog = true;
		void confirmStore
			.warning(
				`Your Trust Score is currently ${trustScoreValue ?? 'unavailable'}. Students with a Trust Score below 60 (Poor or Critical standing) are temporarily restricted from submitting new equipment requests. Please review your account activity and resolve any outstanding issues to restore eligibility.`,
				'Request temporarily on hold',
				'View Trust Score',
				'Dismiss'
			)
	...
	/**
	 * Load the student's trust score snapshot and set the low-trust flag.
	 * Industry-standard: use the same cached statistics endpoint as the dashboard
	 * and fail-open (do not block) if the checks error.
	 */
	async function loadTrustScoreStatus(): Promise<void> {
		try {
			const data = await statisticsAPI.get({ period: '180d' });
			trustScoreValue = data.trustScore?.score ?? null;
			trustTierLabel = data.trustScore?.tierLabel ?? data.trustScore?.tier ?? '';

			// Block when score is Poor (40-59) or Critical (<40)
			hasLowTrust = typeof trustScoreValue === 'number' && trustScoreValue < 60;
		} catch (err) {
			// Fail open: do not restrict requests if stats cannot be read
			trustScoreValue = null;
			trustTierLabel = '';
			hasLowTrust = false;
			console.error('[TRUST] Failed to load trust score:', err);
		}
	}
												...
												<div class="flex-1">
													...
													{#if item.maxQuantityPerRequest && item.available > 0}
														<p class="mt-1 text-[10px] text-gray-500">
															Qty: {item.available} / Max: {item.maxQuantityPerRequest}
														</p>
													...
													{:else if item.available > 0}
														<p class="mt-1 text-[10px] text-gray-500">
															Qty: {item.available} / Max: {item.available}
														</p>
													{/if}
												</div>
											</div>
