<script lang="ts">
	...
	const PAGE_SIZE = 10;
	const initialNotifications = browser ? notificationsAPI.peekCachedList(200, 0) : null;

	let notifications = $state<NotificationRecord[]>(initialNotifications?.notifications ?? []);
	let unreadCount = $state(initialNotifications?.unreadCount ?? 0);
	let loading = $state(!initialNotifications);
	...
	async function loadNotifications(options: { forceRefresh?: boolean; background?: boolean } = {}) {
		const background = options.background === true;
		if (!background) loading = true;
		try {
			const data = await notificationsAPI.list(200, 0, { forceRefresh: options.forceRefresh });
	...
	$effect(() => {
		if (initialNotifications) {
			void loadNotifications({ forceRefresh: true, background: true });
			return;
		}
