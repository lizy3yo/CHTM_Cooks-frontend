<script lang="ts">
	...
	type Tab = 'all' | 'pending' | 'fulfillment' | 'borrowed' | 'unresolved' | 'history';
	let activeTab = $state<Tab>('all');
	let overdueOnly = $state(false);

	let highlightedRequestId = $state<string | null>(null);
	let historySubTab = $state<'all' | 'completed' | 'resolved' | 'cancelled' | 'done'>('all');
	...
	function getWorkflowFilter(): WorkflowFilter {
		...
		if (activeTab === 'fulfillment') return 'fulfillment';
		if (activeTab === 'borrowed' && overdueOnly) return 'overdue';
		if (activeTab === 'borrowed' && !overdueOnly) return 'borrowed';
		if (activeTab === 'unresolved') return 'unresolved';
		if (activeTab === 'history') {
			if (historySubTab === 'done') return 'resolved_completed';
	...
	const activeFilterBadge = $derived.by(() => {
		...
		switch (filter) {
			...
			case 'unresolved': 
				return { label: 'Unresolved Only', bg: 'bg-orange-50', text: 'text-orange-700', ring: 'ring-orange-600/10', btn: 'text-orange-500', btnHoverBg: 'hover:bg-orange-100', btnHoverText: 'hover:text-orange-700' };
			case 'resolved_completed': 
				return { label: 'Resolved & Completed', bg: 'bg-emerald-50', text: 'text-emerald-700', ring: 'ring-emerald-600/10', btn: 'text-emerald-500', btnHoverBg: 'hover:bg-emerald-100', btnHoverText: 'hover:text-emerald-700' };
			case 'cancelled': 
				return { label: 'Cancelled / Declined', bg: 'bg-gray-100', text: 'text-gray-700', ring: 'ring-gray-600/10', btn: 'text-gray-500', btnHoverBg: 'hover:bg-gray-200', btnHoverText: 'hover:text-gray-700' };
		}
	});
				...
				<div class="flex flex-1 items-center gap-2">
					...
					<div class="relative max-w-md flex-1">
						...
						<input
							...
							class="block w-full rounded-xl border border-gray-300 bg-white py-2.5 pl-9 pr-3 text-sm shadow-sm focus:border-pink-500 focus:ring-2 focus:ring-pink-100 focus:outline-none"
						/>
					</div>
				</div>
				<div class="flex shrink-0 items-center gap-2">
					{#if activeTab === 'pending' && selectedRequests.length > 0}
						<span class="text-xs font-medium text-gray-500">{selectedRequests.length} selected</span>
						<button
							onclick={bulkApprove}
							disabled={bulkActionInFlight}
							class="rounded-lg bg-green-600 px-3 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-green-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-green-600 disabled:opacity-50"
						>
						...
						<button
							...
							disabled={bulkActionInFlight}
							class="rounded-lg bg-white px-3 py-2.5 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 disabled:opacity-50"
						>
							Decline
						</button>
						<button
							onclick={() => (selectedRequests = [])}
							class="rounded-lg px-3 py-2.5 text-sm font-semibold text-gray-500 hover:text-gray-700"
						>
							Clear
						</button>
					{:else}
