...
/**
 * Status thresholds mirror determineStatus() in the items API:
 *   quantity === 0  → 'Out of Stock'
 *   quantity <= 5   → 'Low Stock'
 *   quantity > 5    → 'In Stock'
 *
 * We use an aggregation pipeline update so MongoDB can evaluate the
 * post-increment quantity in a single atomic round-trip via $cond.
 */
const STATUS_PIPELINE_EXPR = {
	$cond: [
		{ $lte: ['$quantity', 0] },
		'Out of Stock',
		{
...
export async function decrementInventoryForBorrow(
	...
	for (const item of items) {
		// Pipeline update: atomically decrement quantity and recompute status
		// in a single operation so the two fields are never out of sync.
		const result = await inventoryCollection.updateOne(
			...
			[
				...
				{
					$set: { status: STATUS_PIPELINE_EXPR }
				}
			]
		);
			...
			for (const rollback of decremented) {
				await inventoryCollection.updateOne(
					...
					[
						{
							$set: {
								quantity: { $add: ['$quantity', rollback.quantity] },
								updatedAt: new Date()
							}
						},
						{
...
export async function incrementInventoryOnReturn(
	...
	for (const item of items) {
		// Pipeline update: atomically restore quantity and recompute status.
		await inventoryCollection.updateOne(
			{ _id: item.itemId },
			[
				{
