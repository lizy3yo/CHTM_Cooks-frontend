...
export const POST: RequestHandler = async (event) => {
	...
	try {
		...
		if (!updated) {
			// Best-effort rollback: state-race detected after increment, restore decrement.
			for (const item of borrowRequest.items) {
				await inventoryCollection.updateOne(
					{ _id: item.itemId, quantity: { $gte: item.quantity } },
					[
						{ $set: { quantity: { $subtract: ['$quantity', item.quantity] }, updatedAt: new Date() } },
						{ $set: { status: { $cond: [{ $lte: ['$quantity', 0] }, 'Out of Stock', { $cond: [{ $lte: ['$quantity', 5] }, 'Low Stock', 'In Stock'] }] } } }
					]
				);
			}
