...
export const DELETE: RequestHandler = async (event) => {
	...
	try {
		...
		// Restore inventory stock atomically (quantity + status recalculated in one pipeline update).
		const inventoryCollection = db.collection<any>('inventory_items');
		await incrementInventoryOnReturn(inventoryCollection, updated.items);
