...
export const POST: RequestHandler = async (event) => {
	...
	try {
		...
		// Trust score validation: block requests for students with poor or critical trust scores
		// Industry standard: poor (<60) and critical (<40) trust scores indicate high risk
		const usersCollection = db.collection<User>('users');
		...
		if (studentUser && typeof studentUser.trustScore === 'number' && studentUser.trustScore < 60) {
			const trustTier = studentUser.trustScore < 40 ? 'Critical' : 'Poor';
			logger.warn('Borrow request blocked due to low trust score', {
				userId: user.userId,
				trustScore: studentUser.trustScore,
				trustTier
			});
			return json({
				error: `Unable to submit request. Your current trust score is ${Math.round(studentUser.trustScore)} (${trustTier}). Please improve your borrowing history by returning items on time and resolving any outstanding obligations.`
			}, { status: 403 });
		}
		...
		try {
			...
			// Roll back the stock decrement so quantity + status stay consistent.
			await incrementInventoryOnReturn(inventoryCollection, newRequest.items);
			throw dbError;
		}
