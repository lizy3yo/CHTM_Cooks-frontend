...
async function recordFailedLogin(
	...
	try {
		...
		await db
			...
			.createIndex({ occurredAt: 1 }, { expireAfterSeconds: 7 * 24 * 60 * 60, background: true });
	} catch {
		// Non-critical
	}
}
...
/**
 * Record a successful login event to the auth_events audit collection.
 * Fire-and-forget — never blocks the response.
 */
async function recordLoginEvent(
	...
	try {
		...
		await db
			...
			.createIndex({ occurredAt: 1 }, { expireAfterSeconds: 90 * 24 * 60 * 60, background: true });
	} catch {
		// Non-critical
	}
}
...
export const POST: RequestHandler = async (event) => {
	...
	try {
		...
		// Record login audit event (fire-and-forget)
