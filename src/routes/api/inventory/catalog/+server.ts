...
/**
 * Derive the display status from live quantity data rather than the persisted
 * `status` field. The stored field can lag behind if a write path failed to
 * update it, so we always recompute here as the authoritative source for
 * catalog consumers. Thresholds match determineStatus() in the items API.
 */
function deriveStatus(currentCount: number, archived: boolean): InventoryItemResponse['status'] {
	if (archived) return 'Archived' as InventoryItemResponse['status'];
	if (currentCount <= 0) return 'Out of Stock' as InventoryItemResponse['status'];
	if (currentCount <= 5) return 'Low Stock' as InventoryItemResponse['status'];
	return 'In Stock' as InventoryItemResponse['status'];
}
...
function toItemResponse(item: InventoryItem): InventoryItemResponse {
	...
	return {
		...
		status: deriveStatus(currentCount, item.archived),
...
export const GET: RequestHandler = async (event) => {
	...
	try {
		...
		const required = url.searchParams.get('required') || 'all';
		...
		const cacheKey = `inventory:catalog:${cachePartition}:${search}:${category}:${availability}:${required}:${sortBy}:${page}:${limit}`;
